...
public class HuffmanCompress {
	 ...
	 * freing the memory
	 *********************************************************************************/
	public static void initHuffmanCompress() {
		int i;
		cnt = 0;
		if (Root != null)
			fredfs(Root);
	...
	/*
	 * public static void main (String[] args) { initHuffmanCompress();
	 ...
	 * according the fakezip.txt file initHuffmanCompress();
	 * 
	 * }
	 */

	public static void beginHuffmanCompress(String input) {
		initHuffmanCompress();
		CalFreq(input); // calculate the frequency of each digit
		MakeNode(); // makeing corresponding nodes
		if (cnt > 1)
			dfs(Root, ""); // dfs to make the codes
		fakezip(input); // fake zip file which will have the binary of the input
                        // to fakezipped.txt file
                        
		realzip("fakezipped.txt", input.substring(0, input.length() - 4) + ".sfc"); // making the real zip
